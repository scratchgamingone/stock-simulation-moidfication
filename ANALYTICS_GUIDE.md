...
## Statistics Primer (For Stats Majors)

The Analytics page now computes a direct set of descriptive and risk statistics from portfolio return series:

- Mean daily return: central tendency of daily simple returns
- Median daily return: robust center (less sensitive to outliers)
- Daily volatility: sample standard deviation of returns
- Annualized volatility: daily volatility scaled by $\sqrt{252}$
- Skewness: asymmetry of the return distribution
- Excess kurtosis: tail heaviness relative to Gaussian baseline
- VaR 95%: 5th percentile loss threshold (historical method)
- CVaR 95%: expected loss conditional on being in the 5% worst tail
- Win rate: fraction of days with positive return
- Trend $R^2$: goodness-of-fit from linear regression on portfolio value through time

### Formula Reference

For daily returns $r_1, ..., r_n$:
...
$$
\sigma_{ann} = s\sqrt{252}
$$

$$
	ext{VaR}_{95} = |Q_{0.05}(r)|,
\quad
	ext{CVaR}_{95} = \left|E[r\mid r \le Q_{0.05}(r)]\right|
$$

## Recommended Reusable Functions

A reusable statistics utility has been added at src/util/statisticsToolkit.ts.

Recommended functions to reuse throughout the codebase:
...
- trimmedMean(values, trimFraction)
- geometricMean(values)
- medianAbsoluteDeviation(values)
- zScores(values)
- skewness(values)
- excessKurtosis(values)
- covariance(a, b)
- correlation(a, b)
...
- rollingStandardDeviation(values, windowSize)
- valueAtRisk(returns, confidenceLevel)
- conditionalValueAtRisk(returns, confidenceLevel)
- downsideDeviation(returns, targetReturn)
- sortinoRatio(returns, riskFreeRatePerPeriod, targetReturn)
- beta(assetReturns, benchmarkReturns)
- informationRatio(assetReturns, benchmarkReturns)
- standardError(values)
- confidenceIntervalMean(values, confidenceLevel)
- oneSampleTStatistic(values, hypothesizedMean)
